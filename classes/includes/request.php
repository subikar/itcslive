...

{
...

	{
		...
		if (isset($GLOBALS['_IREQUEST'][$name]['SET.'.$hash]) && ($GLOBALS['_IREQUEST'][$name]['SET.'.$hash] === true)) {
			...
			$var = IRequest::_cleanVar($var, $mask, $type);
...

		{
			...
			if (isset($input[$name]) && $input[$name] !== null) {
				...
				$var = IRequest::_cleanVar($input[$name], $mask, $type);
				...
				if (get_magic_quotes_gpc() && ($var != $default) && ($hash != 'FILES')) {
					...
					$var = IRequest::_stripSlashesRecursive( $var );
			...
			elseif ($default !== null) {
				...
				$var = IRequest::_cleanVar($default, $mask, $type);
...

	{
		...
		return IRequest::getVar($name, $default, $hash, 'int');
...

	{
		...
		return IRequest::getVar($name, $default, $hash, 'float');
...

	{
		...
		return IRequest::getVar($name, $default, $hash, 'bool');
...

	{
		...
		return IRequest::getVar($name, $default, $hash, 'word');
...

	{
		...
		return IRequest::getVar($name, $default, $hash, 'cmd');
...

	{
		...
		return (string) IRequest::getVar($name, $default, $hash, 'string', $mask);
...

	{
		...
		$result = IRequest::_cleanVar($input, $mask);
		...
		if (get_magic_quotes_gpc() && ($hash != 'FILES')) {
			...
			$result = IRequest::_stripSlashesRecursive( $result );
...

	{
		...
		foreach ($array as $key => $value) {
			...
			IRequest::setVar($key, $value, $hash, $overwrite);
...

	{
		...
		if(!IRequest::getVar( $token, '', $method, 'alnum' )) {
...
;				$mainframe->redirect($return, JText::_('SESSION_EXPIRED'));
...

	{
		...
		IRequest::_cleanArray( $_FILES );

		IRequest::_cleanArray( $_ENV );

		IRequest::_cleanArray( $_GET );

		IRequest::_cleanArray( $_POST );

		IRequest::_cleanArray( $_COOKIE );

		IRequest::_cleanArray( $_SERVER );



		if (isset( $_SESSION )) {

			IRequest::_cleanArray( $_SESSION );
